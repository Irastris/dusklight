...
static absl::flat_hash_map<dKy_tevstr_c*, presentation_lighting_history> s_presentationLightingHistory;
static absl::flat_hash_map<J3DModel*, presentation_model_lighting_binding> s_presentationLightingBindings;
...
static presentation_lighting_history& get_presentation_lighting_history(dKy_tevstr_c* tevstr) {
    const uint64_t tick = dusk::interp::sim_tick_seq();
...
static void record_presentation_primary_light(dKy_tevstr_c* tevstr, const cXyz& actorPosition, bool cameraRelative, u8 initTimer) {
...
static void capture_presentation_room_lights(dKy_tevstr_c* tevstr) {
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        sample.cameraRelative = i == 1 && dKy_SunMoon_Light_Check() && sample.worldPosition.abs(moonPosition) < 1.0f;
...
static cXyz sample_presentation_value(const cXyz& previous, const cXyz& current, bool interpolate, f32 step) {
    if (!interpolate) {
        return current;
    }
    cXyz result;
    dusk::interp::lerp(result, previous, current, step);
    return result;
}

static cXyz sample_presentation_primary_light(const presentation_lighting_history& history, const view_class& view, f32 step) {
    const auto& current = history.current.primary;
    const auto& previous = history.previous.primary;
    const bool interpolate = previous.valid && !history.current.discontinuous && previous.space == current.space;

    if (current.space == presentation_primary_light_space::world) {
        return sample_presentation_value(previous.worldPosition, current.worldPosition, interpolate, step);
    }

    const cXyz forward = presentation_camera_forward(view);
    const cXyz residual = sample_presentation_value(previous.cameraResidual, current.cameraResidual, interpolate, step);
    if (current.space == presentation_primary_light_space::camera_eye) {
        return view.lookat.eye + forward * 180.0f + residual;
    }

    const cXyz actorPosition = sample_presentation_value(previous.actorPosition, current.actorPosition, interpolate, step);
...
static void apply_presentation_lighting(void* userWork) {
    ...
    cMtx_multVec(viewMtx, &primaryWorld, &tevstr->mLightObj.getLightInfo()->mLightPosition);
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        const bool interpolate = previous.valid && !history.current.discontinuous && previous.cameraRelative == current.cameraRelative;
        cXyz worldPosition = sample_presentation_value(previous.worldPosition, current.worldPosition, interpolate, step);
        if (current.cameraRelative) {
            const cXyz cameraOffset = sample_presentation_value(previous.cameraOffset, current.cameraOffset, interpolate, step);
            worldPosition = view->lookat.eye + cameraOffset;
        }
        cXyz worldDirection = sample_presentation_value(previous.worldDirection, current.worldDirection, interpolate, step);
...
static void schedule_presentation_lighting(J3DModel* model, dKy_tevstr_c* tevstr) {
    ...
    binding.applied = false;
    dusk::interp::add_model_interpolation_callbacks(model, &apply_presentation_lighting, &restore_presentation_lighting, model);
}
#endif
...
void dScnKy_env_light_c::settingTevStruct_plightcol_plus(cXyz* pos_p, dKy_tevstr_c* tevstr_p,
    ...
    if (pos_p != NULL) {
        ...
        IF_DUSK(record_presentation_primary_light(tevstr_p, *pos_p, sp40 != 0, init_timer));
    }
}
...
#if TARGET_PC
void dScnKy_env_light_c::setLightTevColorType_MAJI(J3DModel* model_p, dKy_tevstr_c* tevstr_p) {

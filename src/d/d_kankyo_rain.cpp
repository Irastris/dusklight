...
static void dKy_set_eyevect_calc(camera_class* i_camera, Vec* o_out, f32 param_2, f32 param_3) {
    cXyz calc;
    view_class* const view = DUSK_IF_ELSE(dKyr_draw_view(i_camera), &i_camera->view);
    get_vectle_calc(&view->lookat.eye, &view->lookat.center, &calc);
    o_out->x = view->lookat.eye.x + calc.x * param_2;
    o_out->y = (view->lookat.eye.y + calc.y * param_3) - 200.0f;
    o_out->z = view->lookat.eye.z + calc.z * param_2;
}
...
void dKy_set_eyevect_calc2(camera_class* i_camera, Vec* o_out, f32 param_2, f32 param_3) {
    ...
    pos.x = view->lookat.center.x - view->lookat.eye.x;
    if (param_3 != 0.0f) {
        pos.y = view->lookat.center.y - view->lookat.eye.y;
    } else {
        pos.y = 0.0f;
    }
    pos.z = view->lookat.center.z - view->lookat.eye.z;

    vectle_calc(&pos, &calc);

    o_out->x = view->lookat.eye.x + calc.x * param_2;
    o_out->y = view->lookat.eye.y + calc.y * param_3;
    o_out->z = view->lookat.eye.z + calc.z * param_2;
...
#if TARGET_PC
    if (rain_packet->raincnt > 0) {
        s_rain.capture(rain_packet->raincnt, 300.0f, [&](int i) {
...
#if TARGET_PC
    if (housi_packet->mHousiCount > 0) {
        s_housi_world.capture(housi_packet->mHousiCount, 300.0f, [&](int i) {
            return housi_packet->mHousiEff[i].mBasePos + housi_packet->mHousiEff[i].mPosition;
        });
        s_housi_scale.capture(housi_packet->mHousiCount, 1.0f, [&](int i) {
...
#if TARGET_PC
    if (snow_packet->field_0x6d88 > 0) {
        s_snow_pos.capture(snow_packet->field_0x6d88, 200.0f, [&](int i) {
            return snow_packet->mSnowEff[i].mPosition;
        });
        s_snow_base.capture(snow_packet->field_0x6d88, 300.0f, [&](int i) {
...
#if TARGET_PC
    if (packet->mCount > 0) {
        s_cloud.capture(packet->mCount, 300.0f, [&](int i) {
...
#if TARGET_PC
    s_vrkumo.capture(100, 1000.0f, [&](int i) {
...
static void dKyr_draw_rev_moon(Mtx drawMtx, u8** tex) {
    ...
    dKankyo_sunlenz_Packet* lenz_packet = g_env_light.mpSunLenzPacket;
    camera_class* camera = (camera_class*)dComIfGp_getCamera(0);
    view_class* const view = DUSK_IF_ELSE(dKyr_draw_view(camera), &camera->view);
    cXyz pos[4];

    u16 date = dComIfGs_getDate();
    cXyz sp60 = view->lookat.eye + g_env_light.moon_pos;
    sp60.y = view->lookat.eye.y - g_env_light.moon_pos.y;

    cXyz moon_pos;
    moon_pos.x = sp60.x - view->lookat.eye.x;
    moon_pos.y = sp60.y - view->lookat.eye.y;
    moon_pos.z = sp60.z - view->lookat.eye.z;
    ...
    if ((draw_sun | draw_moon) != 0) {
        ...
        if (g_env_light.base_light.mColor.r == 0 && stage_type != ST_ROOM) {
            ...
            if (strcmp(dComIfGp_getStartStageName(), "F_SP200") == 0 && dComIfG_play_c::getLayerNo(0) == 0) {
                ...
                spB4 = view->lookat.eye + envlight->moon_pos;
                moon_pos.x = spB4.x - view->lookat.eye.x;
                moon_pos.y = spB4.y - view->lookat.eye.y;
                moon_pos.z = spB4.z - view->lookat.eye.z;
            }
        }

        if (strcmp(dComIfGp_getStartStageName(), "F_SP103") == 0 && dKy_daynight_check()) {
            spB4.x = 3900.0f + view->lookat.eye.x;
            spB4.y = 8052.0f + view->lookat.eye.y;
            spB4.z = -9072.0f + view->lookat.eye.z;
        ...
        if (rain_packet->raincnt != 0) {
            ...
            for (int i = 0; i < rain_packet->raincnt; i++) {
                ...
                if (!(rain_packet->mRainEff[i].mAlpha <= 0.0f)) {
                    ...
                    IF_DUSK(s_rain.try_read(i, &sp3C));
...
void dKyr_drawSibuki(Mtx drawMtx, u8** tex) {
    ...
    cLib_addCalc(&rain_packet->mSibukiAlpha, alpha, 0.2f, 30.0f IF_DUSK(* dusk::game_clock::original_frames()), 0.001f);
            ...
            dComIfGp_roomControl_getStayNo() == 0 || dComIfGp_roomControl_getStayNo() == 11)
        {
            ...
            for (int i = 0; i < 1; i++) {
                ...
                for (int j = 0; j < housi_packet->mHousiCount; j++) {
                    ...
                    IF_DUSK(s_housi_world.try_read(j, &spD0));
                    ...
                    block_14:
                        ...
                        cXyz scale = housi_packet->mHousiEff[j].mScale;
                        IF_DUSK(s_housi_scale.try_read(j, &scale));
                        f32 temp_f28 = (var_f27 * 0.2f) * cM_fsin(scale.x * 5.0f);
                        f32 temp_f30 = (var_f27 * 0.2f) * cM_fcos(scale.y * 6.0f);
                                    ...
                                    housi_packet->mHousiEff[j].mStatus == 3)
                                {
                                    ...
                                    if (housi_packet->mHousiEff[j].field_0x38 > 32765.0f) {
                                        cLib_addCalc(&housi_packet->mHousiEff[j].field_0x44,
                                                        -16384.0f, 0.1f, 500.0f IF_DUSK(* dusk::game_clock::original_frames()), 0.0001f);
                                    } else {
                                        cLib_addCalc(&housi_packet->mHousiEff[j].field_0x44,
                                                        16384.0f, 0.1f, 500.0f IF_DUSK(* dusk::game_clock::original_frames()), 0.0001f);
                                    }
            ...
            if (!g_env_light.camera_water_in_status) {
                ...
                if (tex[0] != NULL) {
                    ...
                    if (g_env_light.field_0xe92 == 0 && sp4C == 0) {
                        cLib_addCalc(&S_fubuki_ratio, 0.0f, 0.25f, 0.1f IF_DUSK(* dusk::game_clock::original_frames()), 0.001f);
                        ...
                        cLib_addCalc(&S_fubuki_ratio, 1.0f, 0.25f, 0.1f IF_DUSK(* dusk::game_clock::original_frames()), 0.001f);
                    }
                    ...
                    for (int i = 0; i < sp54; i++) {
                        for (int j = 0; j < 2; j++) {
                            ...
                            if (g_env_light.mpSnowPacket->mSnowEff[i].mStatus != 0) {
                                ...
                                if (j == 0) {
                                    sp7C = snow_packet->mSnowEff[i].mPosition;
                                    IF_DUSK(s_snow_pos.try_read(i, &sp7C));
                                    ...
                                    sp7C = snow_packet->mSnowEff[i].mBasePos;
                                    IF_DUSK(s_snow_base.try_read(i, &sp7C));
...
void dKyr_drawStar(Mtx drawMtx, u8** tex) {
    ...
    if (star_packet->mEffectNum != 0) {
        ...
        if (strcmp(dComIfGp_getStartStageName(), "F_SP200") == 0 && dComIfG_play_c::getLayerNo(0) == 0) {
            ...
            moon_pos = view->lookat.eye + envlight->moon_pos;
            if (sp38) {
                moon_pos.x = 3900.0f + view->lookat.eye.x;
                moon_pos.y = 8052.0f + view->lookat.eye.y;
                moon_pos.z = -9072.0f + view->lookat.eye.z;
            }
        }
        ...
        spBC.x = view->lookat.eye.x;
        spBC.y = view->lookat.eye.y;
        spBC.z = view->lookat.eye.z;
    ...
    if (cloud_packet->mCount > 0) {
        ...
        for (int i = 0; i < cloud_packet->mCount; i++) {
            ...
            if (!(cloud_packet->mCloudEff[i].mAlpha <= 0.01f)) {
                ...
                IF_DUSK(s_cloud.try_read(i, &sp5C));
            ...
            for (k = 0; k < 100; k++) {
                ...
                if (!(vrkumo_packet->mVrkumoEff[k].mAlpha <= 0.0000000001f) && (pass != 0 || !(vrkumo_packet->mVrkumoEff[k].mAlpha < 0.45f))) {
                    ...
                    if (!(vrkumo_packet->mVrkumoEff[k].mAlpha <= 0.000001f)) {
                        ...
                        IF_DUSK(s_vrkumo.try_read(k, &spFC));
            ...
            if (effect->mStatus != 0) {
                ...
                if (!(temp_f30 > 9000.0f)) {
                    ...
                    if (!(sp54 <= 0.000001f)) {
                        ...
                        cLib_addCalc(&effect->field_0x2c, sp5C, 0.5f, 0.1f IF_DUSK(* dusk::game_clock::original_frames()), 0.01f);

...
struct SkeletonSnapshot {
    ...
    f32 morphStep{};
    f32 animationBlend{};
    dusk::interp::matrix::MatrixSample baseMatrix;
    ...
    std::vector<dusk::interp::matrix::MatrixSample> jointLocalMatrices;
...
void publish_model_matrices(J3DModel* model) {
    J3DModelData* modelData = model->getModelData();
    for (u16 i = 0; i < modelData->getJointNum(); ++i) {
        dusk::interp::override_presentation_mtx(model->getAnmMtx(i), model->getAnmMtx(i));
    }
    for (u16 i = 0; i < modelData->getWEvlpMtxNum(); ++i) {
        dusk::interp::override_presentation_mtx(
...
bool evaluate_captured_pose(J3DModel* model, SkeletonRecord* record, f32 step) {
    ...
    for (u16 jointIndex : current.jointOrder) {
        Mtx localMatrix;
        dusk::interp::matrix::interpolate(localMatrix,
...
dusk::interp::anim::State presented_state(const FrameState& frame) {
    return {
        .frame = frame.frame,
        .rate = frame.rate,
        .loop = static_cast<float>(frame.loop),
        .end = static_cast<float>(frame.end),
        .attribute = frame.attribute,
    };
}

dusk::interp::anim::State presented_state(const AnimationSource& source) {
    return {
        .frame = source.frame,
        .rate = source.attribute == J3DFrameCtrl::EMode_LOOP_REVERSE ? -1.0f : 1.0f,
        .loop = 0.0f,
        .end = static_cast<float>(source.frameMax),
...
bool interpolate_animation_source(
    ...
    return dusk::interp::anim::try_present(previous.frame, presented_state(current),
                                              step, frame);
}
...
void capture_base_matrix(J3DModel* model, dusk::interp::matrix::MatrixSample* baseMatrix) {
    Mtx identity;
    Mtx combined;
...
bool evaluate_morph(J3DModel* model, SkeletonRecord* record, f32 step) {
    ...
    const bool animationChanged = previous.primaryAnimation != current.primaryAnimation ||
                                  previous.secondaryAnimation != current.secondaryAnimation;
    const float previous_frame = animationChanged ? current.frame.frame - current.frame.rate
                                                  : previous.frame.frame;
    if (!dusk::interp::anim::try_present(previous_frame, presented_state(current.frame),
                                            step, &frame))
    {
        return false;
    }
...
void store_snapshot(J3DModel* model, SkeletonSnapshot&& snapshot) {
    const uint64_t sequence = dusk::interp::sim_tick_seq();
    ...
    if (record.scheduledSequence != sequence) {
        dusk::interp::add_presentation_callbacks(
...
namespace dusk::interp::skeleton {

void begin_sim_tick() {
    const uint64_t sequence = interp::sim_tick_seq();
